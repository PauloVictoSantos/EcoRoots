...
function NavItem({ section, isActive, onClick }: { section: typeof sections[0]; isActive: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all ${isActive ? 'bg-[#58D68D]/10 text-[#58D68D] border border-[#58D68D]/20' : 'text-[#9CA3AF] hover:text-white hover:bg-white/4'
      }`}>
